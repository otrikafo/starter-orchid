<template>
  <div class="edit-bien-container">
    <h1 class="edit-bien-title">Modifier un Bien Immobilier</h1>
    <form @submit.prevent="submit" class="edit-bien-form">
      ...
      <div class="form-group">
        ...
        <img v-if="bien.image_principale" :src="`/storage/${bien.image_principale}`" alt="Image Principale Actuelle" class="current-image">
      </div>
      <div class="form-group">
        <label for="images_secondaires" class="form-label">Images secondaires (plusieurs images possibles)</label>
        <input type="file" id="images_secondaires" name="images_secondaires[]" multiple @input="form.images_secondaires = Array.from($event.target.files)" class="form-file-input" accept="image/*">
        <div v-if="form.errors.images_secondaires" class="error">{{ form.errors.images_secondaires }}</div>
      </div>
      <button type="submit" class="form-button" :disabled="form.processing">Modifier</button>
    </form>
  </div>
</template>
...
const form = useForm({
  titre: props.bien.titre,
  description: props.bien.description,
  prix: props.bien.prix,
  adresse: props.bien.adresse,
  ville: props.bien.ville,
  superficie: props.bien.superficie,
  nombre_pieces: props.bien.nombre_pieces,
  nombre_chambres: props.bien.nombre_chambres,
  nombre_salles_de_bain: props.bien.nombre_salles_de_bain,
  type_bien: props.bien.type_bien,
  type_transaction: props.bien.type_transaction,
  image_principale: null, // Pour la nouvelle image (si modifiée)
  images_secondaires: [], // Pour les nouvelles images secondaires (si modifiées)
});
...
const submit = () => {
  ...
  formData.append('nombre_salles_de_bain', form.nombre_salles_de_bain);
  formData.append('type_bien', form.type_bien);
  formData.append('type_transaction', form.type_transaction);
  if (form.image_principale) { // Only append if a file is selected
    formData.append('image_principale', form.image_principale);
  }
  // Append secondary images
  form.images_secondaires.forEach((image) => {
    formData.append('images_secondaires[]', image); // Append each image with the same name array-style
  });

  form.post(route('agence.biens.update', { bien: props.bien.id }), formData, {
    headers: {
    ...
    onSuccess: () => {
      ...
      // Inertia.visit(route('agence.biens.index'));
    },
    onError: (errors) => {
      form.processing = false;
      form.errors = errors;
